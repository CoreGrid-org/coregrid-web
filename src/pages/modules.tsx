  ...
  return (
    <Layout
      ...
      <PageHeader
        ...
        title="10 modules. One asset management platform."
        lead="CoreGrid brings every aspect of asset lifecycle management into one integrated platform - from
              registration and maintenance through to AI-assisted decisions, reporting and full lifecycle
              accountability."
      />
      ...
      <section className="cg-section cg-section--tight">
        <div className="cg-container">
          <Reveal>
            <SectionHeader
              eyebrow="The Stack"
              title="Four layers. One unified platform."
              description="Each layer provides the data and capabilities that the layers above it rely on. Remove any layer and the ones above cannot function correctly."
            />
          </Reveal>
        </div>
      </section>
      ...
      {layers.map((layer, layerIndex) => (
        <section
          key={layer.num}
          id={`layer-${layer.num}`}
          className={`cg-section ${layerIndex % 2 === 1 ? 'cg-section--alt' : ''}`}>
          <div className="cg-container">
            <Reveal>
              <div className={styles.layerHeader}>
                <div className={styles.layerHeading}>
                  <div className={styles.layerBadgeRow}>
                    <span className={styles.layerBadge}>{layer.num}</span>
                  ...
                  <h2 className={`cg-heading ${styles.layerTitle}`}>{layer.title}</h2>
                </div>
              ...
              <div
                className={`${styles.moduleGrid} ${
                  layer.modules.length === 2 ? styles.moduleGridTwo : styles.moduleGridThree
                }`}>
                {layer.modules.map((mod) => (
                  <IconCard key={mod.id} icon={mod.icon} number={mod.id} title={mod.name} description={mod.desc} />
                ))}
              </div>
            </Reveal>
          </div>
          ...
          {layerIndex < layers.length - 1 && (
            <Reveal>
              <div className={styles.connector}>
                ...
                <span className={styles.connectorLabel}>Builds on Layer {layer.num}</span>
              </div>
            </Reveal>
          )}
        </section>
          ...
          <Reveal>
            <div className={`cg-panel ${styles.ctaBanner}`}>
              <div className={styles.ctaContent}>
                <h2 className={`cg-heading ${styles.ctaTitle}`}>Ready to explore the full feature set?</h2>
              ...
              <Link className="cg-btn cg-btn--primary" to="/features">
                View all features
